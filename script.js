...
// Credenciales de tu proyecto TrucoVNZL
...
// --- FUNCIÓN PARA GUARDAR DATOS CON VALIDACIÓN DE APODO ÚNICO ---
async function guardarUsuario() {
    ...
    try {
        // 1. Verificar si el apodo ya existe en la colección "jugadores"
        const q = query(collection(db, "jugadores"), where("apodo", "==", apodo));
        const querySnapshot = await getDocs(q);

        if (!querySnapshot.empty) {
            alert("El apodo '" + apodo + "' ya está en uso. Por favor, elige otro.");
            return; 
        }

        // 2. Si es único, guardar el nuevo jugador
        ...
        alert("¡Registro exitoso en TRUCOVNZL! Tu apodo '" + apodo + "' ha sido registrado.");
        cerrarFormulario();
    } catch (error) {
